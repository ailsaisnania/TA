...
Penulis menggunakan rujukan jurnal dari Melo et al. dalam judul \textit{“A Low-Cost IoT System for Real-Time Monitoring of Climatic Variables and Photovoltaic Generation for Smart Grid Application”} \cite{Melo2021} untuk menentukan batas jumlah data maksimal pada halaman website. Jurnal ini membahas pengembangan dasbor pemantauan cuaca secara \textit{real-time} melalui MQTT dan membuat visualisasi dengan Chart.js. Dalam pengembangannya, dashboard ini hanya menampilkan 1440 titik data untuk menjaga kelancaran \textit{rendering}, kemudian mengganti data terlama saat terdapat titik baru. 

\begin{longtable}{
		|p{\dimexpr.35\linewidth-2\tabcolsep-1.25\arrayrulewidth}|
	...
	\multicolumn{1}{c|}{\textbf{\textit{Library/ Tools}}} & 
	\multicolumn{1}{c|}{\textbf{Studi Kasus}} \\ \hline
	\endfirsthead
...
Penulis meninjau berbagai sumber pustaka akademik yang membahas topik serupa yaitu perbandingan antara \textit{library} visualisasi JavaScript, sebagai landasan sekaligus bahan perbandingan untuk melakukan penelitian ini. Studi-studi ini melakukan penelitian dengan berbagai metode dan berbagai \textit{library} JavaScript selain ChartJS, Highcharts, dan D3.js. 
...
\begin{longtable}{|p{2.5cm}|p{2.3cm}|p{3cm}|p{4.5cm}|}
	\caption{Perbandingan Penelitian Library Visualisasi JavaScript}
	\label{tab:comparison_library} \\
	\hline
	\multicolumn{1}{|c|}{\textbf{Judul}} & 
	\multicolumn{1}{c|}{\textbf{Library}} & 
	\multicolumn{1}{c|}{\textbf{Metric Komparasi}} & 
	\multicolumn{1}{c|}{\textbf{Hasil}} \\ \hline
	\endfirsthead
	\hline
	\multicolumn{1}{|c|}{\textbf{Judul}} & 
	\multicolumn{1}{c|}{\textbf{Library}} & 
...
\multicolumn{1}{c|}{\textbf{Hasil}} \\ \hline	
	...
	\textit{Analysis of Data Visualization Tools and Approaches} [18] 
	& D3.js, Chart.js, Chartist.js, Google Chart 
	& Popularitas \textit{Library}, Kemudahan Penggunaan, Pendekatan grafis (SVG atau Canvas), \textit{Built-in Charts }\& \textit{Customization} 
	& D3.js lebih dari 80.000 GitHub \textit{stars} dan 123 kontributor. Chart.js dengan lebih dari 40.000 stars dan 284 kontributor. Chartist.js dengan 11.000 stars dan 67 kontributor. \\
	\hline
	
	\textit{Comparative Study of Data Visualization Tools} [19] 
	& Microsoft Excel, D3.js, Chart.js, Highcharts, Leaflet, Dygraphs, FusionCharts, Google Charts, Timeline.js 
	& Ketersediaan \textit{open-source}, kebutuhan pengetahuan pemrograman, format output grafis 
	& Library \textit{open-source:} D3, Chart.js, Leaflet, Dygraphs, Google Charts, Timeline.js. D3, Chart.js, Highcharts, Leaflet, Dygraphs, Google Charts, FusionCharts perlu kemampuan \textit{coding}. Mayoritas berbasis JavaScript. Microsoft Excel hanya chart statis, D3 mendukung chart interaktif, Chart.js fokus animasi chart, Highcharts, Dygraphs, Google Charts unggul dalam interaktivitas, Leaflet untuk peta interaktif, FusionCharts mendukung chart dan peta, Timeline.js untuk timeline interaktif. \\
	\hline
	
	\textit{Scalability of JavaScript Libraries for Data Visualization} [4] 
	& Apexcharts, Frappe Charts, TeeCharts JS, jQuery 
	& ANOVA-test pada ukuran dataset 
	& Ukuran dataset mempengaruhi waktu render di semua \textit{chart}. Tidak ada perbedaan signifikan waktu \textit{render} antara diagram garis dan batang pada dataset 5.000 titik. \\
	\hline
	
	\textit{Comparison of D3.JS and Chart.JS as Visualization Tools} [20] 
	& D3.JS, Chart.JS 
	& Waktu eksekusi, \textit{load time}, penggunaan memori 
	& D3 lebih cepat dari Chart.js untuk dataset besar. Chart.js \textit{crash} saat merender 1.000.000 data. Chart.js lebih efisien dalam memori jika tiap titik data dirender sebagai elemen SVG individual. \\
	\hline
	
	\textit{A performance investigation into JavaScript visualization libraries with the focus on render time and memory usage} [3] 
	& D3, ECharts, CanvasJS, Chartist, Highcharts, Plotly 
	& ANOVA-Test dan Tukey-test pada waktu \textit{render} 
	& Ukuran dataset mempengaruhi waktu \textit{render}. Semua \textit{library} mengalami peningkatan waktu \textit{render} seiring bertambahnya data, meskipun ada beberapa anomali. \\
	\hline
	
	\textit{A Comparison of Performance and Scalability of Chart Generation for Javascript Data Visualization Libraries} [21] 
	& Chart.js, ApexCharts, Chartist, Billboard 
	& Performa render 
	& Chart.js mampu memproses hingga 1.000.000 data tanpa \textit{crash}. Chartist cepat tetapi gagal merender dataset lebih dari 100.000 data. ApexCharts dan Billboard mengalami peningkatan waktu respon secara eksponensial. \\
	\hline
	
	\textit{Beyond Alternative Text and Tables: Comparative Analysis of Visualization Tools and Accessibility Methods} [22] 
	& Highcharts, Vega-Lite, Visa Chart, amCharts, FusionCharts, AnyChart 
	& Aksesibilitas Chart, cara menampilkan data real-time 
	& Highcharts memiliki dukungan aksesibilitas terbaik berupa navigasi keyboard multi-level, mode kontras tinggi, dan sonifikasi. Mendukung pembaruan data \textit{real-time} dengan kontrol jumlah data agar kompleksitas render dapat dikurangi. \\
	\hline
...
Berdasarkan penelitian-penelitian sebelumnya, setiap studi memiliki metrik penilaiannya masing-masing dalam melakukan komparasi, seperti kemudahan penggunaan, performa \textit{rendering}, ketersediaan jenis \textit{chart}, waktu eksekusi, konsumsi memori, serta metrik lainnya sebagaimana dijelaskan pada paragraf dan tabel sebelumnya. Oleh karena itu, dalam penelitian ini, penulis melakukan komparasi terhadap tiga \textit{library} visualisasi berbasis JavaScript, yaitu Chart.js, D3.js, dan Highcharts, dengan fokus pada studi kasus \textit{dashboard website} untuk menampilkan data sensor IoT secara \textit{real-time}.

Pengujian dilakukan dengan mengacu pada metrik-metrik yang telah digunakan dalam penelitian terdahulu, yaitu performa \textit{rendering} untuk memastikan kecepatan dan kelancaran tampilan data yang terus diperbarui, penggunaan memori untuk melihat efisiensi dalam penanganan elemen-elemen visual yang dihasilkan, serta alokasi CPU untuk meninjau sejauh mana beban pemrosesan memengaruhi kinerja sistem secara keseluruhan. Selain itu, skalabilitas masing-masing \textit{library} juga diamati untuk memastikan kemampuan visualisasi dalam menangani pertambahan volume data sensor IoT yang dapat berkembang signifikan dalam implementasi nyata. Dengan pendekatan ini, diharapkan hasil pengujian dapat menjadi acuan praktis bagi pengembang dalam memilih \textit{library} yang paling sesuai untuk membangun \textit{dashboard} IoT yang responsif, stabil, dan efisien.
...
\begin{enumerate}
	...
	\item \textit{Buffer} dan \textit{cache} internal yang menyimpan data sementara dari tab \textit{website}.
\end{enumerate}

\subsubsection{Scalabilitas}
Konsep skalabilitas menjelaskan kemampuan suatu sistem untuk beradaptasi dengan pertambahan elemen atau objek, menangani peningkatan beban kerja secara efisien, serta dapat diperluas sesuai kebutuhan. Dalam proses perancangan sistem, skalabilitas sering menjadi faktor utama yang harus dipenuhi untuk mengukur kemampuan sistem dalam menerima sumber daya \cite{Bondi2000}.
...
\textit{Task Manager} merupakan ekstensi Chrome yang dapat digunakan untuk memantau dan mengelola berbagai proses yang berjalan di dalam browser. Beberapa hal yang diukur dan ditampilkan oleh \textit{Chrome Task Manager} antara lain:
\begin{enumerate}
	\item Penggunaan CPU menunjukkan seberapa banyak prosesor yang digunakan oleh setiap proses. 
	\item \textit{Memory Footprint} mengukur jumlah memori yang digunakan oleh setiap proses, termasuk tab, ekstensi, dan plugin. Hal ini membantu dalam mengidentifikasi proses yang memakan banyak RAM.
	\item \textit{Network} memantau aktivitas jaringan dari setiap proses sehingga penggunaan bandwith dapat terpantau. 
	\item \textit{Process ID} merupakan id yang dapat digunakan untuk pelacakan dan diagnostik lebih lanjut.
\end{enumerate}

\subsection{\textit{Use Case Diagram}}
\textit{Use case diagram} digunakan untuk menggambarkan bagaimana sistem berfungsi dari sudut pandang penggunanya. Diagram ini menunjukkan pola interaksi yang biasa terjadi antara pengguna dan sistem. Dalam pembahasannya, pengguna sering disebut sebagai aktor, yakni pihak yang berperan langsung dalam menggunakan sistem untuk mencapai suatu tujuan. Aktor dapat berupa perorangan, sebuah organisasi, atau sistem eksternal, yang berinteraksi dengan sistem atau aplikasi secara langsung \cite{Evi}.

\subsection{\textit{Sequence Diagram}}
\textit{Sequence diagram} adalah salah satu diagram \textit{UML (Unified Modeling Language)} yang digunakan untuk memperlihatkan alur komunikasi antar objek dalam sistem seiring berjalannya waktu. Diagram ini menyusun urutan interaksi berdasarkan skenario tertentu dan mengaitkannya dengan operasi yang dimiliki oleh masing-masing objek dalam \textit{class diagram}. Diagram ini cukup populer karena bentuk visualnya yang mudah dipahami dan kemampuannya untuk menggambarkan sebagian perilaku sistem secara spesifik. Terdapat beberapa elemen di dalam \textit{sequence diagram} \cite{Fedaghi}. Elemen-elemen tersebut antara lain : 
\begin{enumerate}
	\item Aktor adalah pihak di luar sistem, contohnya adalah pengguna, yang berinteraksi langsung dengan sistem untuk menjalankan suatu proses.
	
	\item Objek atau \textit{instance} adalah bagian yang menggambarkan komponen dalam sistem yang berperan dalam suatu skenario atau alur tertentu.
	
	\item \textit{Lifeline} adalah garis vertikal yang menunjukkan objek aktif selama proses interaksi berlangsung.
	
	\item Pesan ditampilkan dalam bentuk panah antar objek yang merepresentasikan komunikasi, baik yang berjalan secara sinkron maupun asinkron.
	
	\item \textit{Activation bar} adalah blok kecil di atas \textit{lifeline} yang menunjukkan bahwa objek tersebut sedang menjalankan aktivitas tertentu pada waktu itu.
	
	\item \textit{Fragments} bersifat opsional dan biasanya digunakan untuk menunjukkan percabangan alur, seperti kondisi pilihan, aksi opsional, atau perulangan.
